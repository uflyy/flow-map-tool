...
const USAMapVisualization = ({ flows }: { flows: FlowRow[] }) => {
  ...
  // Scheme A: log scaling + normalized weight range
  const maxFlow = useMemo(() => {
    if (!flows.length) return 1;
    return Math.max(...flows.map(f => Number(f.displayValue) || 0), 1);
  }, [flows]);
    ...
    <div className="w-full h-full rounded-xl overflow-hidden border border-slate-200 relative">
      ...
      >
        ...
        {flows.map((f, i) => {
          ...
          const t = Math.log1p(v) / Math.log1p(maxFlow); // 0..1
          const weight = minW + t * (maxW - minW);
      ...
      <div className="absolute bottom-2 right-2 bg-white/95 backdrop-blur px-3 py-2 rounded-lg text-xs text-gray-600 shadow-md border border-gray-100 flex flex-col gap-1 pointer-events-none">
        ...
        <div className="mt-1 text-[10px] text-gray-400">Line width ∝ log(flow volume)</div>
      </div>
    </div>
  );
};
...
export default function App() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setError('Auto-load failed. Please check server static file or upload a CSV.');
      } finally {
        setLoading(false);
      }
    };
  ...
  // Map data (Top 200) with robust coordinate parsing + lon fix
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans text-gray-800">
      <header className="bg-slate-900 text-white p-4 shadow-lg sticky top-0 z-50">
        <div className="container mx-auto flex justify-between items-center">
          ...
          <div className="text-xs text-slate-300 hidden md:flex items-center gap-3">
            <div className="flex items-center gap-2">
          ...
          <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-200 grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
            ...
            <div className="md:col-span-2">
              <div className="bg-slate-100 border border-slate-200 px-3 py-2 rounded-lg text-right">
                <span className="block text-[10px] uppercase text-slate-500 font-bold">Total volume</span>
            ...
            <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col">
              <div className="p-3 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                <h3 className="font-semibold text-gray-700 flex items-center gap-2 text-sm">
                  <MapIcon className="w-4 h-4" /> OD Flow Map (Top 200)
                </h3>
